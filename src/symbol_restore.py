...
from src.llm_client import ChatCompletionFailure, DEFAULT_TIMEOUT_S, chat_completion_json
...
def _build_messages(rec: ProblemRecord, issues: list[ValidationIssue]) -> list[dict[str, str]]:
    user = {
        ...
        "hint": "Use error snippets to locate where symbols were dropped.",
    }
    return [
...
def restore_symbols_llm(
    ...
    *,
    ...
    attach_flags = [
        f
        for f in rec.flags
        if f.startswith("missing_image:")
        or f in {"expected_image_missing", "manual_review_required"}
    ]
    ...
    fixed_symbol = original_codes & symbol_codes - remaining_codes
    unfixed_symbol = original_codes & symbol_codes & remaining_codes
