...
if [[ -f "$SERVER_DIR/.env" ]]; then
  ...
  TUTOR_MODEL_FALLBACK_SAVED="$(grep -E '^TUTOR_MODEL_FALLBACK=' "$SERVER_DIR/.env" 2>/dev/null | cut -d= -f2- || true)"
fi
...
TUTOR_MODEL_FALLBACK_VAL="${TUTOR_MODEL_FALLBACK_SAVED:-nvidia/nemotron-3-ultra-550b-a55b:free}"
...
TUTOR_MODEL_FALLBACK=${TUTOR_MODEL_FALLBACK_VAL}
